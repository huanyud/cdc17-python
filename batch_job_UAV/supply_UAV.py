...
def generate_graph(num_of_source=100, num_of_sink=1000, supply_each=10, search_radius=10, plane_size=100):
    G = nx.MultiDiGraph()
    ...
    ''' WARNING: Useful code below. Do not delete.
    ...
    plt.legend((scatter_loc, scatter_uav),
               ('Potential locations', 'UAV anchor points'),
               scatterpoints=1,
               loc=0,
               ncol=1)
    plt.savefig('figure_supply_UAV_spatial.pdf')
    '''
...
outfile = open('result_supply_UAV.txt', 'a')
...
plt.savefig('figure_supply_UAV.pdf', bbox_inches='tight')
