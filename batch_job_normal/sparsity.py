...
def generate_graph(num_of_source, num_of_sink, edge_quota, supply_each=10, arc_prob=0.5):
    G = nx.MultiDiGraph()
...
num_of_source = 200
num_of_sink = 300
edge_quota_list = [10000, 14000, 18000, 22000, 26000, 30000, 34000, 38000, 42000, 46000, 50000]
time_list_our_algo = [0.0] * len(edge_quota_list)
time_list_capacity_scaling = [0.0] * len(edge_quota_list)
[G, grand_total_supply] = generate_graph(num_of_source, num_of_sink, 0)
for kk in range(0, len(edge_quota_list)):
    # Remove edges between sources and sinks
    ebunch = [edge for edge in G.edges(data=True) if edge[0][0] == 's']
    G.remove_edges_from(ebunch)
    
    # Regenerate edges between sources and sinks
    edge_quota = edge_quota_list[kk]
...
outfile = open('result_sparsity.txt', 'a')
...
plt.savefig('figure_sparsity.pdf')
