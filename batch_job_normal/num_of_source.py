...
num_of_source_list = [80, 120, 160, 200, 240]
num_of_sink = 280
supply_each_list = [30, 20, 15, 12, 10]
time_list_our_algo = [0.0] * len(num_of_source_list)
time_list_capacity_scaling = [0.0] * len(num_of_source_list)
for kk in range(0, len(num_of_source_list)):
    tmp_time_cumulative_our_algo = 0.0
    tmp_time_cumulative_capacity_scaling = 0.0
    how_many_iterations = 50
    for itr in range(how_many_iterations):
        num_of_source = num_of_source_list[kk]
    ...
    # Store results
    time_list_our_algo[kk] = tmp_time_cumulative_our_algo / how_many_iterations
    time_list_capacity_scaling[kk] = tmp_time_cumulative_capacity_scaling / how_many_iterations


# Save data to a txt file
outfile = open('result_num_of_source.txt', 'a')
outfile.write('----#source:' + str(num_of_source_list) + ', #sink: ' + str(num_of_sink) + '----\n')
outfile.write('supply_at_each_source: ' + str(supply_each) + '\n')
outfile.write('time_our_algo: ' + str(time_list_our_algo) + ', time_capacity_scaling: ' + str(time_list_capacity_scaling) + '\n')
outfile.close()

# Plot figure
plt.plot(num_of_source_list, time_list_our_algo, 'rs-')
plt.plot(num_of_source_list, time_list_capacity_scaling, 'b^--')
plt.xlabel('Number of agent/source nodes')
plt.ylabel('Time elapsed (sec)')
plt.legend(['New algorithm', 'Capacity scaling'], loc=2)
plt.xlim(75, 245)
plt.xticks(num_of_source_list)
#plt.ylim(50, 300)
#plt.yticks([50, 100, 150, 200, 250, 300])
plt.savefig('figure_num_of_source.pdf')
